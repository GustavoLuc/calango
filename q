...
arquivo = '2025-07-29_17.52.48.ass'
...
fs.readFile(legenda, 'utf8',(err, data)=>{
   ...
   legenda = 'Dialogue: 0,0:00:00.00,4:00:00.00,Titulo,,0,0,0,,{\\c&H00FFFF&\\pos(955,1030)} Inspeção Visual - CPVV - B4 Face Sul'
   ...
   for(let i = 0; i < filtrar2.length; i++){
     ...
     if(filtrar2[i].includes('Depth')){
        ...
        Depth_final =  inicio2[0]+'\\'+inicio2[1]+'}'+posicao_valor2[posicao_depth].replace('N','')
   ...
   if(filtrar2[i].includes('Heading')){
        ...
        Healding_final = inicio[0]+'\\'+inicio[1]+'}'+posicao_valor[posicao_Heading].replace('N','').replace(' deg','°')
   ...
   if(filtrar2[i].includes('Time')){
        ...
        Time_final =  inicio3[0]+'\\'+inicio3[1]+'}'+posicao_valor3[posicao_Time].replace('N','')
    ...
    if(filtrar2[i].includes('Date')){
        ...
        if(posicao4[0].includes('Date')){
           ...
           Date_final = inicio4[0]+'\\'+inicio4[1]+'}'+posicao_valor4[posicao_Date].replace('N','')
